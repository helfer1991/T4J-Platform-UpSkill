...
/**
 * Regra do Tipo de Regimento 2 que implementa TiposRegimentoStrategy
 * @author joaor
 */
public class TipoRegimento2 implements TiposRegimentoStrategy {

    private FabricaRepositorios fabricaRepositorios = new FabricaRepositoriosDB();
    private Plataforma plat = Plataforma.getInstance();

    /**
     * Regista o processo de seriação subjetiva com atribuição obrigatória.
    ...
    public void RegistaProcessoSeriacao(Anuncio anuncio, List<Colaborador> participantes, ArrayList<Classificacao> listaClass) throws Exception {
        ...
        if(pS.getClassificacoes().size()>0){
            RegistaAtribuicao(pS.getClassificacoes().get(0));
        }
    }
